...
# Max redirects
{
  ...
  is(Startsiden::UserAgent->new->max_redirects, 25, 'right value');
  $ENV{MOJO_MAX_REDIRECTS} = 0;
  is(Startsiden::UserAgent->new->max_redirects, 0, 'right value');
}

# Timeouts
{
  is(Startsiden::UserAgent->new->connect_timeout, 2, 'right value'); # 10
  local $ENV{MOJO_CONNECT_TIMEOUT} = 25;
  is(Startsiden::UserAgent->new->connect_timeout,    25, 'right value');
  is(Startsiden::UserAgent->new->inactivity_timeout, 5, 'right value'); # 20
  local $ENV{MOJO_INACTIVITY_TIMEOUT} = 25;
  is(Startsiden::UserAgent->new->inactivity_timeout, 25, 'right value');
  $ENV{MOJO_INACTIVITY_TIMEOUT} = 0;
  is(Startsiden::UserAgent->new->inactivity_timeout, 0, 'right value');
  is(Startsiden::UserAgent->new->request_timeout,    10, 'right value'); # 0
  local $ENV{MOJO_REQUEST_TIMEOUT} = 25;
  is(Startsiden::UserAgent->new->request_timeout, 25, 'right value');
  $ENV{MOJO_REQUEST_TIMEOUT} = 0;
  is(Startsiden::UserAgent->new->request_timeout, 0, 'right value');
}

# Default application
is(Mojo::UserAgent::Server->app,      app, 'applications are equal');
is(Startsiden::UserAgent->new->server->app, app, 'applications are equal');
Mojo::UserAgent::Server->app(app);
is(Mojo::UserAgent::Server->app, app, 'applications are equal');
my $dummy = Mojolicious::Lite->new;
isnt(Startsiden::UserAgent->new->server->app($dummy)->app,
...
$ua = Startsiden::UserAgent->new(ioloop => Mojo::IOLoop->singleton);
...
# Connection limit
$ua = Startsiden::UserAgent->new(max_connections => 2);
my $result;
Mojo::IOLoop->delay(
  sub {
    my $delay = shift;
